...
export default function MarchandTable() {
  ...
  const COLUMNS = [
    ...
    // { label: <span className="text-blue-500">Produits livrés</span>, renderCell: (item) =>item.livraison.length},
    // { label: <span className="text-blue-500">Paye</span>, renderCell: (item) =>item.livraison.length},
    { label: <span className="text-blue-500">Produits livrés</span>, renderCell: (item) =>  
    <button 
      className="text-white bg-orange-500 hover:bg-orange-600 focus:outline-none p-4 rounded-lg"
      onClick={() => handleClick(item)}>{item.livraison.length}
    </button>
    },
  ];  
  ...
  const handleClick = (item) =>{
    ...
    console.log('Les données', item.livraison); 
    const id = JSON.stringify(item.livraison, null, 2);
    console.log('Les données à parser', id);
    router.push({
      pathname: '/boutique/marchandPaiement',
      query: { id }
    });
  }
